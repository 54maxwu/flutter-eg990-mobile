...
//        'highlight: $_isHighlight');
  ...
  Widget _buildItem() {
    return FittedBox(
      fit: BoxFit.contain,
      child: Container(
        constraints: BoxConstraints.tight(Size(56, 80)),
        child: Column(
  ...
  Widget _createImage() {
    String path = (_isHighlight) ? Res.movie_eg_base : Res.movie_eg_dark_base;
    if (widget.categoryEnum.value < 0) {
      if (widget.categoryEnum == MovieCategoryEnum.COLLECT)
        ...
        path = 'assets/images/movie_eg/expand.png';
      else if (widget.categoryEnum == MovieCategoryEnum.FOLD)
        path = 'assets/images/movie_eg/fold.png';
    } else {
