...
//        debugPrint('user agent: ${client.userAgent}\n');
  ...
  Future<Response<dynamic>> get(
    ...
    try {
      ...
      // set option headers
      if (headers != null && userToken != null) {
        Map<String, dynamic> getHeaders = new Map();
        if (headers != null) getHeaders.addAll(headers);
        if (userToken != null) getHeaders['JWT-TOKEN'] = userToken;
        getOptions.headers = getHeaders;
      }
      // send get request
      return await _dio.get(url,
          queryParameters: data, options: getOptions, cancelToken: cancelToken);
    } on DioError catch (e) {
  ...
  Future<Response<dynamic>> post(
    ...
    try {
      Options postOptions = options ?? Options();
      // set option headers
      if (userToken != null) {
        postOptions.headers = {'JWT-TOKEN': userToken};
      }
      // send post request
